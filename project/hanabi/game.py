...
class Game(object):
    ...
    def __satisfyPlayCardRequest(self, data: GameData.ClientPlayerPlayCardRequest):
        ...
        if p.name == data.sender:
            ...
            if self.__gameOver:
                ...
                logging.info("Score: " + str(self.__score) +
                             "; message: " + self.__scoreMessages[self.__score])
    ...
    def __checkGameEnded(self):
        ...
        for player in self.__players:
            ended = ended and ((len(player.hand) < 5 and len(
                self.__players) <= 3) or len(player.hand) < 4)
        if ended:
            score = 0
            for pile in self.__tableCards:
                score += len(pile)
